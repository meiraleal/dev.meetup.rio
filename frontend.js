...
async function fetchAndParseSearchResults({ url }) {
	try {
		const response = await fetch(url);
		const data = `${(await response.text()).slice(18)}`;
		...
		try {
			...
			if (!isExtension) {
				SW = await navigator.serviceWorker.register(
					`${APP.config.BASE_URL}/serviceworker.js`,
					{ type: "classic", scope: "/www/" },
				);
...
const setColorVariables = (root, colors) => {
	...
	Object.keys(colors).forEach((color) => {
		...
		Object.entries(shades).forEach(([shade, percentage]) => {
			...
			root.style.setProperty(`--color-${color}-${shade}`, lightShade);
